...
| `external-cratesio-install-smoke` | External crates.io install smoke | `release-proof` | `cargo xtask cratesio-smoke --version 0.10.0` | Crates.io smoke proves an external install path for a published version, not every downstream feature combination. |
