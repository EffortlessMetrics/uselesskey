#![no_main]

use std::sync::OnceLock;

use libfuzzer_sys::fuzz_target;

use uselesskey::negative::{corrupt_pem, CorruptPem};
use uselesskey::{Ed25519FactoryExt, Ed25519Spec, Factory, Seed};

static GOOD_PEM: OnceLock<String> = OnceLock::new();

fn good_pem() -> &'static str {
    GOOD_PEM
        .get_or_init(|| {
            let fx = Factory::deterministic(Seed::new([7u8; 32]));
            let kp = fx.ed25519("fuzz", Ed25519Spec::new());
            kp.private_key_pkcs8_pem().to_string()
        })
        .as_str()
}

fuzz_target!(|data: &[u8]| {
    let pem = good_pem();

    // Choose corruption based on the first byte.
    let how = match data.first().copied().unwrap_or(0) % 5 {
        0 => CorruptPem::BadHeader,
        1 => CorruptPem::BadFooter,
        2 => CorruptPem::BadBase64,
        3 => CorruptPem::ExtraBlankLine,
        _ => CorruptPem::Truncate {
            bytes: (data.len() % 64),
        },
    };

    let bad = corrupt_pem(pem, how);

    // We don't care if it parses; we care that parsing doesn't panic.
    let _ = pkcs8::DecodePrivateKey::from_pkcs8_pem(&bad)
        .map(|k: ed25519_dalek::SigningKey| drop(k));
});
