...
proptest! {
    ...
    fn output_length_matches_requested(seed in any::<[u8; 32]>(), len in 0usize..256) {
        let out = random_base62(Seed::new(seed), len);
    ...
    fn different_seeds_produce_different_output(
        ...
        let a = random_base62(Seed::new(seed_a), 64);
        let b = random_base62(Seed::new(seed_b), 64);
        prop_assert_ne!(a, b);
    }

    #[test]
    fn zero_length_produces_empty_string(seed in any::<[u8; 32]>()) {
        let out = random_base62(Seed::new(seed), 0);
