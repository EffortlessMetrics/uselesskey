//! Manifest-driven deterministic fixture materialization.
...
fn materialized_fixture_bytes(
    ...
    match spec.kind {
        ...
        MaterializeKind::TokenJwtShape => Ok(fx
            ...
            .as_bytes()
            .to_vec()),
        MaterializeKind::RsaPkcs8Der => rsa_pkcs8_der_fixture(&fx, &label),
        MaterializeKind::RsaPkcs8Pem => rsa_pkcs8_pem_fixture(&fx, &label),
        MaterializeKind::PemBlockShape => pem_block_shape_fixture(spec, &label),
        MaterializeKind::SshPublicKeyShape => Ok(ssh_public_key_shape_fixture(spec, &label)),
...
#[cfg(feature = "rsa-materialize")]
fn rsa_pkcs8_der_fixture(fx: &Factory, label: &str) -> Result<Vec<u8>, MaterializeError> {
    Ok(fx
...
fn rsa_pkcs8_der_fixture(_fx: &Factory, _label: &str) -> Result<Vec<u8>, MaterializeError> {
...
#[cfg(feature = "rsa-materialize")]
fn rsa_pkcs8_pem_fixture(fx: &Factory, label: &str) -> Result<Vec<u8>, MaterializeError> {
    Ok(fx
        .rsa(label, RsaSpec::rs256())
...
#[cfg(not(feature = "rsa-materialize"))]
fn rsa_pkcs8_pem_fixture(_fx: &Factory, _label: &str) -> Result<Vec<u8>, MaterializeError> {
    Err(MaterializeError::InvalidManifest(
        "rsa.pkcs8_pem requires uselesskey-cli feature `rsa-materialize`".to_string(),
    ))
}

fn pem_block_shape_fixture(
    spec: &MaterializeFixtureSpec,
    label: &str,
) -> Result<Vec<u8>, MaterializeError> {
...
fn ssh_public_key_shape_fixture(spec: &MaterializeFixtureSpec, label: &str) -> Vec<u8> {
    let seed = Seed::from_text(&spec.seed);
