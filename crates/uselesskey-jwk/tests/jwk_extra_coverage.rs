...
//! - `negative_value()` across every concrete `PrivateJwk`/`PublicJwk` variant
//!   (RSA, EC, OKP, Oct) and every `NegativeJwk` variant — these arms each
//!   call into the same private helper but with different field lookup orders.
...
#[test]
fn jwks_to_value_returns_json_object_with_keys_array() {
    let jwks = Jwks {
...
#[test]
fn jwks_to_value_matches_display_output_shape() {
    let jwks = Jwks {
...
#[test]
fn negative_jwks_duplicate_key_on_populated_set_repeats_first_entry() {
    let jwks = Jwks {
        keys: vec![
