...
mod rsa_aws_lc_rs_tests {
    ...
    fn test_rsa_deterministic_keys() {
        ...
        aws1.sign(&signature::RSA_PKCS1_SHA256, &rng, msg, &mut sig1).unwrap();
        aws2.sign(&signature::RSA_PKCS1_SHA256, &rng, msg, &mut sig2).unwrap();
    ...
    fn test_rsa_cross_key_verification_fails() {
        ...
        aws_a.sign(&signature::RSA_PKCS1_SHA256, &rng, msg, &mut sig).unwrap();
    ...
    fn test_rsa_signature_tampering_fails() {
        ...
        aws_keypair.sign(&signature::RSA_PKCS1_SHA256, &rng, msg, &mut sig).unwrap();
...
mod ecdsa_aws_lc_rs_tests {
    ...
    fn test_ecdsa_p256_key_pair_conversion() {
        ...
        let msg = b"test message";
        let rng = SystemRandom::new();
        let sig = aws_keypair.sign(&rng, msg).expect("Failed to sign with AWS LC-RS ECDSA key pair");
...
mod ed25519_aws_lc_rs_tests {
    ...
    fn test_ed25519_key_pair_conversion() {
        ...
        // Verify key pair is valid by attempting to sign
        let msg = b"test message";
        let sig = aws_keypair.sign(msg).expect("Failed to sign with AWS LC-RS Ed25519 key pair");
...
mod cross_algorithm_tests {
    ...
    fn test_cross_algorithm_signature_verification_fails() {
        ...
        aws_rsa.sign(&signature::RSA_PKCS1_SHA256, &rng, msg, &mut sig).unwrap();
