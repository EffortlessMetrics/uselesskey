...
assert int(after["summary"]["total"]) == 3422
...
assert sum(removed.values()) == 44, removed
...
core_removed = 0
stale_removed_by_path: Counter[str] = Counter()
for record, count in removed.items():
    ...
    if path in core_paths:
        ...
        assert path is not None, (fields, count)
        stale_removed_by_path[path] += count

assert core_removed == 18, core_removed
assert stale_removed_by_path == Counter(
    {
        "xtask/src/main.rs": 19,
        "crates/uselesskey-core/src/sink/mod.rs": 4,
        "crates/uselesskey-webauthn/src/lib.rs": 2,
        "crates/uselesskey-cli/src/main.rs": 1,
    }
), stale_removed_by_path
assert sum(after_entries.values()) == 3422
